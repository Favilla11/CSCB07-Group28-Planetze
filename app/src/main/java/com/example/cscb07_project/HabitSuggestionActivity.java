...
public class HabitSuggestionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ObjectAnimator Buttongradiant = ObjectAnimator.ofObject(buttonbackground, "color", new ArgbEvaluator(),
                Color.parseColor("#3F51B5"), Color.parseColor("#009999"));
        Buttongradiant.setDuration(2500);
        ...
        personalizedSuggestionAdapter = new ArrayAdapter<>(this, android.R.layout.simple_list_item_1, new ArrayList<String>(getHabitStringList(PersonalizedHabits(info))));
        ...
        getUserInfo(new UserInfoCallback() {
            @Override
            public void onUserInfoRetrieved (Information info){
                if (info != null) {
                    // Process the information
                    Log.d("UserInfo", "Retrieved info: " + info.toString());
                } else {
                    Log.d("UserInfo", "No information found for today.");
                }
            }

            @Override
            public void onError (DatabaseError error){
                Log.e("UserInfo", "Error fetching data: " + error.getMessage());
            }
        });
    }
...
/*    private Information getUserInfo() {
        DatabaseReference infoRef = realtimeRef.child("3rwLTVZ280dKkoGEqKlgOfLv6Rf2").child("userInformation");
 ...
 */
    ...
    public void getUserInfo(UserInfoCallback callback) {
        DatabaseReference habitListRef = realtimeRef.child("3rwLTVZ280dKkoGEqKlgOfLv6Rf2").child("userInformation");
        habitListRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
